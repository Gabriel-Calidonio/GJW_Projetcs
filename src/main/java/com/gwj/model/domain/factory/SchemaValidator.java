...
public class SchemaValidator {
        ...
        try (java.sql.Connection conn = com.gwj.model.dataAccessObject.ConnectionDB.getInstance().getConnection();
            ...
            try {
                if (AppConfig.isPostgreSql()) {
                    stmt.execute("CREATE UNIQUE INDEX IF NOT EXISTS email_unique ON tab_usuario (email)");
                } else {
                ...
                if (e.getErrorCode() == 1061 || "42000".equals(e.getSQLState()) || "42P07".equals(e.getSQLState())) {
    ...
    public static void ensureSettingsTableExists() {
        try (java.sql.Connection conn = com.gwj.model.dataAccessObject.ConnectionDB.getInstance().getConnection();
            ...
            // Cria a tabela tab_setting se ela não existir de acordo com o dialeto
            String createTableSql;
            if (AppConfig.isPostgreSql()) {
                createTableSql = "CREATE TABLE IF NOT EXISTS tab_setting (" +
    ...
    public static void ensureProductImageColumnAndDataExist() {
        try (java.sql.Connection conn = com.gwj.model.dataAccessObject.ConnectionDB.getInstance().getConnection();
            ...
            try {
                if (AppConfig.isPostgreSql()) {
                    stmt.execute("ALTER TABLE tab_produto ADD COLUMN IF NOT EXISTS imagem VARCHAR(255) DEFAULT NULL");
                } else {
                ...
                // Código 1060 indica que a coluna já existe no MySQL
                if (e.getErrorCode() == 1060 || "42S21".equals(e.getSQLState()) || "42701".equals(e.getSQLState())) {
    ...
    public static void ensureOrderTablesExist() {
        try (java.sql.Connection conn = com.gwj.model.dataAccessObject.ConnectionDB.getInstance().getConnection();
            ...
            if (AppConfig.isPostgreSql()) {
                createPedidosSql = "CREATE TABLE IF NOT EXISTS tab_pedidos (" +
                                   "  id BIGSERIAL PRIMARY KEY," +
            ...
            String createItensSql;
            if (AppConfig.isPostgreSql()) {
                createItensSql = "CREATE TABLE IF NOT EXISTS tab_itens_pedido (" +
                                 "  id BIGSERIAL PRIMARY KEY," +
