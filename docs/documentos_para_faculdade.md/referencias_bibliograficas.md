...
* **Onde se aplica no projeto:**
  * Fundamenta diretamente o [DataMapper.java](/src/main/java/com/gwj/model/dataAccessObject/DataMapper.java), o [UnitOfWork.java](/src/main/java/com/gwj/service/transaction/UnitOfWork.java) e o [QueryBuilder.java](/src/main/java/com/gwj/model/dataAccessObject/QueryBuilder.java).
...
* **Onde se aplica no projeto:**
  * **Singleton:** Gerenciamento do ciclo de conexão única no banco em [ConnectionDB.java](/src/main/java/com/gwj/model/dataAccessObject/ConnectionDB.java).
  * **Factory Method / Simple Factory:** Criação e resolução dinâmica de instâncias de entidades via reflexão em [SimpleObjectFactory.java](/src/main/java/com/gwj/model/domain/factory/SimpleObjectFactory.java).
  ...
  * **Strategy / Interceptor:** Interceptação centralizada de requisições e exceções em [GlobalExceptionHandler.java](/src/main/java/com/gwj/controller/GlobalExceptionHandler.java) e [AdminInterceptor.java](/src/main/java/com/gwj/controller/AdminInterceptor.java).
...
* **Onde se aplica no projeto:**
  * **Data Access Object (DAO):** Isolamento e abstração da camada relacional em [GenericRepository.java](/src/main/java/com/gwj/model/repository/GenericRepository.java).
  * **Data Transfer Object (DTO) / Entity Mapper:** Conversão e transporte dinâmico de parâmetros de formulários HTTP para o domínio em [EntityMapper.java](/src/main/java/com/gwj/model/dataTransferObject/EntityMapper.java).
  * **Service Locator / Registry:** Registro e despacho dinâmico de serviços em [ServiceRegistry.java](/src/main/java/com/gwj/service/ServiceRegistry.java).
...
* **Onde se aplica no projeto:**
  * **Princípios SOLID:** Separação estrita de responsabilidades (SRP) entre Controller, Service, Repository, Mapper e Builder; Inversão de Dependências (DIP) usando interfaces genéricas ([IRepository.java](/src/main/java/com/gwj/model/repository/IRepository.java), [IService.java](/src/main/java/com/gwj/service/IService.java)).
...
* **Onde se aplica no projeto:**
  * **Gerenciamento de Recursos com `AutoCloseable`:** Uso de `try-with-resources` no [UnitOfWork.java](/src/main/java/com/gwj/service/transaction/UnitOfWork.java) para fechamento determinístico de transações JDBC.
...
* **Onde se aplica no projeto:**
  * **Entidades e Relacionamentos Ricos:** Modelagem do domínio ([Cliente.java](/src/main/java/com/gwj/model/domain/entities/Cliente.java), [Profissional.java](/src/main/java/com/gwj/model/domain/entities/Profissional.java), [Agendamento.java](/src/main/java/com/gwj/model/domain/entities/Agendamento.java)).
  * **Serviços de Domínio:** Encapsulamento de regras complexas de negócio em [AgendamentoService.java](/src/main/java/com/gwj/service/AgendamentoService.java) (cálculo de slots de horários, colisões e janelas de funcionamento).
...
* **Onde se aplica no projeto:**
  * Modelagem EER, cardinalidades N:N com tabelas associativas (`tab_profissional_endereco`, `tab_agenda_servico`), chaves estrangeiras (`@JoinColumn`) e garantias ACID documentadas no [adr-0001.md](/docs/adr/adr-0001.md).
...
* **Onde se aplica no projeto:**
  * Diagramas de classes de domínio, diagramas estruturais e diagramas de sequência em PlantUML localizados em [diagramas/](/docs/diagramas/).
...
* **Onde se aplica no projeto:**
  * Diagramas de classes de domínio, diagramas estruturais, diagramas de sequêsncia, diagramas de caso de uso, diagramas de atividade e diagramas de estado  localizados em [diagramas/](/docs/diagramas/).
...
* **Onde se aplica no projeto:**
  ...
  * Defesa ativa e armadilhas automatizadas via [HoneypotController.java](/src/main/java/com/gwj/controller/HoneypotController.java).
