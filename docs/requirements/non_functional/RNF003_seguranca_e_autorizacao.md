...
1. **Criptografia de Senhas (Hashing):**
   * As senhas de usuários são submetidas a funções de dispersão criptográfica (SHA-256) via [`PasswordUtil.java`](/src/main/java/com/gwj/controller/PasswordUtil.java).
...
3. **Proteção e Mascaramento de Rotas Administrativas:**
   * O painel administrativo opera sob rota ofuscada (`/MRYnZpAsC9sp/*`) com interceptação mandatória via [`AdminInterceptor.java`](/src/main/java/com/gwj/controller/AdminInterceptor.java).
...
4. **Proteção contra Bots (Honeypot):**
   * Presença de campos armadilha invisíveis via [`HoneypotController.java`](/src/main/java/com/gwj/controller/HoneypotController.java) nos formulários públicos para detecção e descarte automático de submissões automatizadas de spam.
